...
class BBox:
    ...
    @classmethod
    def world2box(cls,bbox):
        cos, sin = np.cos(bbox.o), np.sin(bbox.o)
        transform = np.array([[cos,  sin, 0, bbox.x],
                              [sin, -cos, 0, bbox.y],
                              [0, 0, 1, bbox.z],
                              [0,0,0,1]])
    ...
    @classmethod    
    def pointsinsidebbox(cls,bbox,points):    
        transform = BBox.world2box(bbox)
        points = transform[:3,:3] @ points.T + transform[:3,-1:] 
        index = (np.abs(points[0,:])<bbox.l/2) * (np.abs(points[1,:])<bbox.w/2) * (np.abs(points[2,:])<bbox.h/2)
        return index,points.T[index]
