...
def generate_info(split, filenames, sample=False, compute_statistics=False):
    split_infos = []
    ...
    for filename in tqdm(filenames, desc=f"Generating {split} infos"): 
        ...
        with open(label_file, 'r') as f:  
            for line in f:  
                ...
                if category_name in class_map:                          # Ignore "DontCare labels" 
                    instance = {  
...
def main():
    ...
    if osp.exists(train_text_file):  
        ...
        generate_info(split='train', filenames=train_filenames, sample=args.sample, compute_statistics=True)

    if osp.exists(val_text_file):  
        with open(val_text_file, 'r') as f:  
            val_filenames = [line.strip() for line in f]  
        generate_info(split='val', filenames=val_filenames, sample=args.sample)
