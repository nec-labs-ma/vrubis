...
def kitti_eval(gt_annos,
    ...
    #                         [0.5, 0.25, 0.25, 0.5, 0.25]])
    overlap_0_7 = np.array([[0.7, 0.5, 0.5],   # Car, Pedestrian, Cyclist
                        [0.7, 0.5, 0.5],
                        [0.7, 0.5, 0.5]])

    overlap_0_5 = np.array([[0.5, 0.25, 0.25],  # Car, Pedestrian, Cyclist
                            [0.5, 0.25, 0.25],
                            [0.5, 0.25, 0.25]])
    ...
    for j, curcls in enumerate(current_classes):
        ...
        # prepare results for print
        result += ('{} AP40@{:.2f}, {:.2f}:\n'.format(
            curcls_name, *min_overlaps[:, j, j]))
        # if mAP40_bbox is not None:
        #     result += 'bbox AP40:{:.4f}, {:.4f}, {:.4f}\n'.format(
        #         *mAP40_bbox[j, :, i])
        if mAP40_bev is not None:
            result += 'bev  AP40:{:.4f}, {:4f}\n'.format(*mAP40_bev[j, 0, :])
        if mAP40_3d is not None:
            result += '3d   AP40:{:.4f}, {:4f}\n'.format(*mAP40_3d[j, 0, :])
        # if compute_aos:
        #     result += 'aos  AP40:{:.2f}, {:.2f}, {:.2f}\n'.format(
        #         *mAP40_aos[j, :, i])

            # # prepare results for logger
            for i in range(min_overlaps.shape[0]):  # over IoU sets (0.7-set, 0.5-set)
                postfix = 'strict' if i == 0 else 'loose'
                prefix = f'KITTI/{curcls_name}'
                
                if mAP40_3d is not None:
                    ret_dict[f'{prefix}_3D_AP40_{postfix}'] = mAP40_3d[j, 0, i]
                if mAP40_bev is not None:
                    ret_dict[f'{prefix}_BEV_AP40_{postfix}'] = mAP40_bev[j, 0, i]
                if mAP40_bbox is not None:
                    ret_dict[f'{prefix}_2D_AP40_{postfix}'] = mAP40_bbox[j, 0, i]
            # for idx in range(3):
            #     if i == 0:
            #         postfix = f'{difficulty[idx]}_strict'
            #     else:
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(2):
            postfix = f'{difficulty[idx]}'
            if mAP40_3d is not None:
                print(mAP40_3d.shape)
                ret_dict[f'KITTI/Overall_3D_AP40_{postfix}'] = mAP40_3d[0, idx]
            if mAP40_bev is not None:
